...
fn play_sound_aloud(tone_arrangements : Vec<SoundTone>) {
    ...
    for tone in tone_arrangements {
        ...
        let sound_duration_ms : std::time::Duration = std::time::Duration::from_millis(tone.duration_ms as u64);

        let source = rodio::source::SineWave::new(tone.frequency_hz as f32).take_duration(sound_duration_ms).amplify(1.0);
